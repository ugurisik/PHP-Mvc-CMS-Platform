...
$main_path = "../../../" . str_replace(SITE_URL, "", $_REQUEST["main_path"]);
if (!is_dir($main_path)) {
    mkdir($main_path, 0777, true);
}

$thumbnail_path = "../../../" . str_replace(SITE_URL, "", $_REQUEST["thumbnail_path"]);
...
if ($_REQUEST["resize_to"] > 0) {
    $width = $imgsize[0];
    $height = $imgsize[1];
    if ($width > $_REQUEST["resize_to"]) createThumbnail($main_path, $filename, $main_path, $_REQUEST["resize_to"], 85);
}

if ($bytes > 8) {
    if ((int)$_REQUEST["thumbnail_size"] > 0) createThumbnail($main_path, $filename, $thumbnail_path, $_REQUEST["thumbnail_size"], 85);
} else exit;
...
function rotateImage($image_name, $path, $degree_lvl)
{
    ...
    switch ($image_extension) {
        case "jpg":
            ...
            imagejpeg($rotated_image, "$path/$image_name", 80);
            break;
        case "jpeg":
            header('Content-type: image/jpeg');
            imagejpeg($rotated_image, "$path/$image_name", 80);
...
function addWatermark($watermark, $imageDirectory, $imageName, $x = 0, $y = 0)
{
    if (file_exists($watermark)) {
        ...
        switch ($image_extension) {
            case "jpg":
                header('Content-type: image/jpeg');
                imagejpeg($im, "$imageDirectory/$imageName", 80);
                break;
            case "jpeg":
                header('Content-type: image/jpeg');
                imagejpeg($im, "$imageDirectory/$imageName", 80);
...
echo str_replace(SITE_URL, "", $_REQUEST["main_path"]) . "/" . $filename;
