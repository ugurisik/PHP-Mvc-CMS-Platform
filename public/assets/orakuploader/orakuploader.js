(function($) {
    ...
    $.fn.orakuploader = function(options) {
        ...
        if ($(this).attr('orakuploader') == 'on') {
            ...
            holdername = $(this).replaceWith(getHtml($(this).attr('id'), settings.orakuploader_add_image, settings.orakuploader_add_label, settings.orakuploader_path, settings.orakuploader_accept_types));
        ...
        for (i = 0; i < settings.orakuploader_attach_images.length; i++) {
            ...
            $(clone).html("<div class='picture_delete'>" + settings.orakuploader_file_delete_label + "</div><img src='" + image + "' alt='111111' onerror=this.src='" + settings.orakuploader_path + "/images/no-image.jpg' class='picture_uploaded'/> <input type='hidden' value='" + image.replace(settings.orakuploader_siteurl, "") + "' name='" + settings.orakuploader_field_name + "[]' />");
            $(clone).attr('id', image);
    ...
    function upload(file, clone, place, holder, settings) {
        ...
        xhr.onreadystatechange = function() {
            ...
            if (xhr.readyState == 4) {
                ...
                $(clone).html("<div class='picture_delete'>" + settings.orakuploader_file_delete_label + "</div>" + rotation_html + "<img src='" + settings.orakuploader_siteurl + "/" + xhr.responseText + "' alt='" + xhr.responseText + "' onerror=this.src='" + settings.orakuploader_path + "/images/no-image.jpg' class='picture_uploaded'/> <input type='hidden' value='" + xhr.responseText + "' name='" + settings.orakuploader_field_name + "[]' />");
...
function getHtml(name, add_image, add_label, path, accept_type) {
    return '<div id="' + name + '_to_clone" class="clone_item"><div class="multibox file"><div class="loading"><img src="' + path + '/images/loader.gif" alt="loader"/></div></div></div>\
		<div id="' + name + 'DDArea">\
			...
			<input type="file" class="' + name + 'Input asd orakuploaderFileInput" accept="' + accept_type + '" multiple/>\
			<div class="clear"> </div>\
		</div>';
